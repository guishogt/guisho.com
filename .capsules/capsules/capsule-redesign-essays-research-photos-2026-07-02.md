...
## RenderATL speaking entry (2026-08-17)
SHIPPED (commit 72db25f): /speaking/render-atl-2026/ — "Beyond UX and DX: Let's Greet AX" (RenderATL, Atlanta, Aug 12-13 2026, delivered a capela after AV failure). Personal note on top, The Talk + Synopsis sections, 6 photos in the page bundle (full-room shot slimmed 2.5MB→200KB), self-contained 11MB HTML slide deck at /speaking/render-atl-2026/slides.html. Patterns learned: .html files inside a leaf bundle become page resources and DON'T publish raw — host raw HTML under static/ (merges into the same URL dir); static files bypass the theme, so GA must be injected into their <head> by hand (done, + real <title> replacing "Bundled Page" for legible GA reports).

## 10 new postcards + year/tag navigation (2026-08-27)
SHIPPED (commit 40cca6d): 10 picture posts from Luis's late-2024/25 photos (Lafayette, NOLA, Paço de Arcos, Ponte 25 de Abril, Fátima, Texans@NRG, Central Park, brisket, Texas snow, Adobe HQ) — dates pulled from EXIF (fixed two label guesses), HEIC decoded via pillow-heif (system lacks HEVC codecs; magick/ffmpeg both fail), originals + 1600w variants on guisho-media. Two near-duplicate NOLA frames (identical EXIF timestamp) merged into one entry. New tags: louisiana, fatima, bbq. Postcards feed: year bands + anchor year strip + travel-tag strip (tags with >=2 postcards, #-junk hidden) collapsed to exactly two rows via JS measurement (pure CSS max-height guess failed), "all tags" toggle. Prototyped 3 variants on ports 1314-16 (A in-page anchors / B JS filter chips / C real /postcards/YYYY/ pages + plate term pages); Luis picked A; B/C theme copies + year stubs deleted. Rejected-for-now: plate-style tag term pages (C) — revisit if tag browsing feels flat. AWS root session renewed via remote-browser OAuth: user pasted the 127.0.0.1 callback URL, delivered locally with curl to the aws login listener.
